...
var (
	binary = filepath.Join(os.Getenv("GOPATH"), "bin/cmc_exporter")
)

const (
	address = "localhost:19599"
)

func TestHasMetrics(t *testing.T) {
	if _, err := os.Stat(binary); err != nil {
		t.Skipf("cmc_exporter binary not available, try to run `make build` first: %s", err)
	}
